...
export default function CaseStudyImage({ src, alt = '', className = '' }) {
  ...
  return (
    <div className="cs-video">
      <img src={src} alt={alt} className={`cs-mockup ${className}`.trim()} />
      ...
      {expanded && createPortal(
        <div className="cs-video-overlay" onClick={() => setExpanded(false)}>
          ...
          {/* Fixed to the viewport, not the frame, so it stays put while a tall image scrolls. */}
          <div className="cs-video-controls cs-video-controls--fixed" onClick={(e) => e.stopPropagation()}>
            <button type="button" className="cs-video-btn" onClick={() => setExpanded(false)} aria-label="Minimize image">
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
